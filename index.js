...
const config = {
    ...
    nomer: 'wa.me/6282397075910',
    aktif: '24 JAM',
    tanggal: `TANGGAL: ${moment().format('DD')} ${bulan} ${moment().format('YYYY')}`,
    waktu: time
}

const { BELL, tanggal, waktu, instagram, whatsapp, youtube, nomer, aktif, ontime } = config
...
client.on('message-new', async (m) => {
   ...
   switch (prefix) {
       case 'help':
           client.sendMessage(id, help.help(id, BELL, tanggal, whatsapp, youtube, instagram, nomer),MessageType.text)
           break      
      case 'donate':
           client.sendMessage(id, donate.donate(id, BELL, tanggal),MessageType.text)
           break          
      case 'info':
           client.sendMessage(id, info.info(id, BELL, tanggal),MessageType.text)
           break             
       case 'nulis':
           nulis(value)
               .then(data => {
